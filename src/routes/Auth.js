...
const Auth = ()=>{
  ...
  const {email,password} = input //input.email 이런식으로 쓰기 힘드니까 구조분해할당 함
  ...
  return(
    <>
      ...
      <Form.Group className="mb-3" controlId="email">
        ...
        <Form.Control type="email" name="email" value={email} placeholder="name@example.com" onChange={onChange}/>
      </Form.Group>
    ...
    <div>{error}</div>
    <hr/>
    <Button type='button' variant="info" onClick={onGoogleSignIn}>{newAccount ? "구글로 회원가입" : "구글로 로그인"}</Button>
    <hr/>
